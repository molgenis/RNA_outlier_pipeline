...
#' Uses output fraser rds objects from merge_counts.R as input.
#' It calculates the Intron Jaccard Index for each gene in the dataset.
#' It performs normalization of the Intron Jaccard Index using the autoencoder.
#' It calculates P-values for each gene and saves all results in the fraser rds object.
#' 28-10-2023
...
saveFraserDataSet(fds, dir=workdir, name="fraser_out2")
...
saveFraserDataSet(fds, dir=workdir, name="fraser_out3")

#fds <- annotateRangesWithTxDb(fds, txdb=txdb, orgDb=orgDb)
#onderstaande annotate manier leek te werken toen ik het handmatig in de singularity shell deed op basis van fraser_out4
fds <- annotateRanges(fds, GRCh=37)

saveFraserDataSet(fds, dir=workdir, name="fraser_out4")

#saveFraserDataSet(fds, dir=workdir, name="fraser_out5")
